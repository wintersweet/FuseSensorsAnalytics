...
static NSString *const kSAEventPropertyDeepLinkURL = @"deeplinkUrl";

static NSString *const kSavedDeepLinkInfoFileName = @"latestUtms";
...
- (void)trackDeeplinkMatchedResult:(NSURL *)url result:(NSDictionary *)result interval:(NSTimeInterval)interval errorMsg:(NSString *)errorMsg {
    ...
    props[@"eventDuration"] = [NSString stringWithFormat:@"%.0f", interval*1000];
    props[@"deeplinkOptions"] = result[@"page_params"];
    props[@"deeplinkMatchFailReason"] = errorMsg.length ? errorMsg : nil;
